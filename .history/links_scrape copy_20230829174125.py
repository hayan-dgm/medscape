...
urls = []
# Read the Excel file
excel_filename = '2MedScape_Antidotes .xlsx'
        ...
        try:
            ...
            if response.status_code == 200:
                try:
                    ...
                    head = soup.find('div',id="drugdbmain")
                    contents =head.findChildren()
                    for c in contents:
                        p = c.findAll("p")
                        for par in p:
                            if str(par.text).startswith("To view formulary information first create a list of plans."):
                                print("0"*30)
                                break
                            print(par.text)
                            print("_"*30)
